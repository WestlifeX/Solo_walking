from second_order.LIPM_to_whole_body import interpolate_lipm_traj
from second_order.LIPM_to_whole_body import compute_foot_traj
import second_order.plot_utils as plot_utils
import matplotlib.pyplot as plt
import numpy as np

def mpc_to_tsid(conf):

    mpc_data_files = {'mpc': conf.DATA_FILE_LIPM_MPC,
                      'rmpc':conf.DATA_FILE_LIPM_RMPC,
                      'smpc':conf.DATA_FILE_LIPM_SMPC}
    for i in mpc_data_files:
        # READ COM-COP TRAJECTORIES
        data = np.load(mpc_data_files[i])
        ...
        com, dcom, ddcom, cop, contact_phase, foot_steps_ctrl = \
            interpolate_lipm_traj(conf.T_step, conf.nb_steps, conf.dt_mpc,
                    dt_ctrl, conf.h, conf.g, com_state_x, com_state_y, cop_ref,
                                                                   cop_x, cop_y)
        # number of time steps for traj-opt
        N  = conf.nb_steps * int(round(conf.T_step/conf.dt_mpc))
        N_ctrl = int((N*conf.dt_mpc)/dt_ctrl) # number of time steps for TSID
        foot_steps_RF = foot_steps[::2,:]     # assume first foot step right

        # COMPUTE FEET TRAJECTORIES
        x_RF, dx_RF, ddx_RF = compute_foot_traj(foot_steps_RF, N_ctrl, dt_ctrl,
                                        conf.T_step, conf.step_height, 'stance')
        foot_steps_LF = foot_steps[1::2,:]
        x_LF, dx_LF, ddx_LF = compute_foot_traj(foot_steps_LF, N_ctrl, dt_ctrl,
                                         conf.T_step, conf.step_height, 'swing')
        # SAVE COMPUTED TRAJECTORIES IN NPY FILES FOR TSID
        if i == 'mpc':
            np.savez(conf.DATA_FILE_TSID_MPC, com = com, dcom = dcom,
                     ddcom = ddcom, x_RF = x_RF, dx_RF = dx_RF, ddx_RF = ddx_RF,
                                    x_LF = x_LF, dx_LF = dx_LF, ddx_LF = ddx_LF,
                                       contact_phase = contact_phase, cop = cop)
        elif i == 'rmpc':
            np.savez(conf.DATA_FILE_TSID_RMPC, com = com, dcom = dcom,
                                      ddcom = ddcom, x_RF = x_RF, dx_RF = dx_RF,
                                    ddx_RF = ddx_RF, x_LF = x_LF, dx_LF = dx_LF,
                      ddx_LF = ddx_LF, contact_phase = contact_phase, cop = cop)
        else:
            np.savez(conf.DATA_FILE_TSID_SMPC, com = com, dcom = dcom,
                     ddcom = ddcom, x_RF = x_RF, dx_RF = dx_RF, ddx_RF = ddx_RF,
                                    x_LF = x_LF, dx_LF = dx_LF, ddx_LF = ddx_LF,
                                       contact_phase = contact_phase, cop = cop)
        # PLOT STUFF
