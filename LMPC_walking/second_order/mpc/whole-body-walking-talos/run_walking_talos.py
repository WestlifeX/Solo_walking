from plan_LIPM_talos import run_nominal_mpc
from LIPM_to_TSID_talos import mpc_to_tsid
...
if(PLOT_TORQUES):
    ...
    for i in range(tsid.robot.na):
        ...
        if np.max(np.abs(tau_normalized))>0.5:
            plt.plot(time, tau_normalized, alpha=0.5,
                                                    label=tsid.model.names[i+2])
