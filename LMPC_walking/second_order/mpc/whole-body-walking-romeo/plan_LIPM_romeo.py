...
# --------------------------------------------
[P_ps, P_vs, P_pu, P_vu] = motion_model.compute_recursive_matrices(conf.dt_mpc,
                                                             conf.g, conf.h, N)
[Q, p_k] = cost_function.compute_objective_terms(conf.alpha, conf.beta,
           conf.gamma,conf.T_step, nb_dt_per_step, N, conf.step_length,
                 step_width, P_ps, P_pu, P_vs, P_vu, x_0, y_0, cop_ref)
...
[A_com , b_com] = constraints.add_CoM_constraints(N, y_0, P_ps, P_pu,
                                                 conf.com_constraint)
...
# -------------------------------------------------------------------------
[com_state_x, com_state_y] = motion_model.compute_recursive_dynamics(P_ps, P_vs,
                                                    P_pu, P_vu, N, x_0, y_0, U)
com_state_x = np.vstack((x_0, com_state_x))
com_state_y = np.vstack((y_0, com_state_y))

np.savez(conf.DATA_FILE_LIPM, com_state_x=com_state_x, com_state_y=com_state_y,
               cop_ref=cop_ref, cop_x=cop_x, cop_y=cop_y, foot_steps=foot_steps)
# ------------------------------------------------------------------------------
# visualize your open-loop trajectory:
# ------------------------------------------------------------------------------
"""
time = np.arange(0, round(N*conf.dt_mpc, 2), conf.dt_mpc)
min_admissible_CoP = cop_ref - np.tile([foot_length/2, foot_width/2], (N+1,1))
max_admissible_cop = cop_ref + np.tile([foot_length/2, foot_width/2], (N+1,1))
CoM_constraint_vector = np.tile(conf.com_constraint, (N,1))

# time vs CoP and CoM in x: 'A.K.A run rabbit run !'
# -------------------------------------------------
ZMP_x_fig = plt.figure()
plot_utils.plot_x(conf.plot_legend, time, N+1, min_admissible_CoP,
                  max_admissible_cop, cop_x, com_state_x, cop_ref)

# time VS CoP and CoM in y: 'A.K.A what goes up must go down'
# ----------------------------------------------------------
ZMP_y_fig = plt.figure()
plot_utils.plot_y(conf.plot_legend, time, N, min_admissible_CoP,
        max_admissible_cop, cop_y, com_state_y, cop_ref, CoM_constraint_vector)
