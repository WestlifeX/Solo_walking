from plan_robust_LIPM_talos import run_rmpc
from LIPM_to_TSID_talos import rmpc_to_tsid
import second_order.plot_utils as plut
...
for k in range(conf.total_simulations):
    ...
    for i in range(-N_pre, N):
        ...
        if i==0:
            ...
            if contact_phase[i] != contact_phase[i-1]:
                print("Time %.3f Changing contact phase from %s to %s"%(t,
                                          contact_phase[i-1], contact_phase[i]))
        ...
        if i<0:
            tsid.set_com_ref(com_pos_ref[:,0], 0*com_vel_ref[:,0],
                                                             0*com_acc_ref[:,0])
        elif i<N:
            tsid.set_com_ref(com_pos_ref[:,i], com_vel_ref[:,i],
                                                               com_acc_ref[:,i])
        ...
        if(sol.status!=0):
            ...
            break
        if norm(v,2)>40.0:
            print("Time %.3f Velocities are too high, stop everything!"%(t),
                                                                        norm(v))
            break
        ...
        if i%conf.PRINT_N == 0:
            ...
            if tsid.formulation.checkContact(tsid.contactRF.name, sol) and i>=0:
                print("\tnormal force %s: %.1f"%(tsid.contactRF.name.ljust(20,
                                                               '.'), f_RF[2,i]))
            if tsid.formulation.checkContact(tsid.contactLF.name, sol) and i>=0:
                print("\tnormal force %s: %.1f"%(tsid.contactLF.name.ljust(20,
                                                               '.'), f_LF[2,i]))
            print("\ttracking err %s: %.3f"%(tsid.comTask.name.ljust(20,'.'),
        ...
        t += conf.dt
        """
        frameIndex_r = tsid.model.getFrameId('gripper_right_fingertip_2_link')
        frameIndex_l = tsid.model.getFrameId('gripper_left_fingertip_2_link')

        pin.forwardKinematics(tsid.model, data, q)
        pin.updateFramePlacement(tsid.model, data, frameIndex_r)
        finalPosEff_r = np.array(data.oMf[frameIndex_r].translation.T.flat)
        pin.updateFramePlacement(tsid.model, data, frameIndex_l)
        finalPosEff_l = np.array(data.oMf[frameIndex_l].translation.T.flat)
        """
        #display collisions
        if i>2*int(round(conf.T_step/conf.dt)) and \
                                            i<6*int(round(conf.T_step/conf.dt)):
            if com_pos_ref[1,i] < -conf.com_constraint:
                print('oh oh .. I hit my right arm')
                tsid.gui.setHighlight('world/right_wall', 1)
                tsid.gui.refresh()
            else:
                tsid.gui.setHighlight('world/right_wall', 0)
                tsid.gui.refresh()

            if com_pos_ref[1,i] > conf.com_constraint:
                print('oh oh .. I hit my left arm')
                tsid.gui.setHighlight('world/left_wall', 1)
                tsid.gui.refresh()
            else:
                tsid.gui.setHighlight('world/left_wall', 0)
                tsid.gui.refresh()
...
#        ax[i].plot(time[:N], cop_ref[i,:], label='CoP ref '+str(i))
        if i==0:
            ax[i].plot([time[0], time[-1]], [-conf.lxn, -conf.lxn], ':',
                                                        label='CoP Lim '+str(i))
            ax[i].plot([time[0], time[-1]], [conf.lxp, conf.lxp], ':',
                                                        label='CoP Lim '+str(i))
        elif i==1:
            ax[i].plot([time[0], time[-1]], [-conf.lyn, -conf.lyn], ':',
                                                        label='CoP Lim '+str(i))
            ax[i].plot([time[0], time[-1]], [conf.lyp, conf.lyp], ':',
                                                        label='CoP Lim '+str(i))
        ax[i].set_xlabel('Time [s]')
...
if(PLOT_TORQUES):
    ...
    for i in range(tsid.robot.na):
        tau_normalized = 2*(tau[i,:]-tsid.tau_min[i]) / \
                                          (tsid.tau_max[i]-tsid.tau_min[i]) - 1
...
if(PLOT_JOINT_VEL):
    plt.figure()
    for i in range(tsid.robot.na):
        v_normalized = 2*(v_log[6+i,:]-tsid.v_min[i]) / \
                                              (tsid.v_max[i]-tsid.v_min[i]) - 1
