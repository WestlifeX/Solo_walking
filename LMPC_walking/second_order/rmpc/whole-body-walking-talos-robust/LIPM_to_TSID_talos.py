...
def rmpc_to_tsid(conf):

    # READ COM-COP TRAJECTORIES COMPUTED WITH ROBUST MPC
    ...
    com, dcom, ddcom, cop, contact_phase, foot_steps_ctrl = \
        interpolate_lipm_traj(conf.T_step, conf.nb_steps, conf.dt_mpc, dt_ctrl,
                conf.h, conf.g, com_state_x, com_state_y, cop_ref, cop_x, cop_y)
    ...
    np.savez(conf.DATA_FILE_TSID, com=com, dcom=dcom, ddcom=ddcom, x_RF=x_RF,
              dx_RF=dx_RF, ddx_RF=ddx_RF, x_LF=x_LF, dx_LF=dx_LF, ddx_LF=ddx_LF,
